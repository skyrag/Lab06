...
public class JCalculator extends JFrame
{
  ...
  public JCalculator() 
  {
    ...
    addOperatorButton("MR", 0, 1, Color.RED, new OpMr(state));

    // Stockage d'une valeur en memoire
    addOperatorButton("MS", 1, 1, Color.RED, new OpMs(state));

    // Backspace
    addOperatorButton("<=", 2, 1, Color.RED, new OpBackspace(state) );

    // Mise a zero de la valeur courante + suppression des erreurs
    addOperatorButton("CE", 3, 1, Color.RED, new OpCe(state, jNumber));

    // Comme CE + vide la pile
    addOperatorButton("C",  4, 1, Color.RED, new OpC(state, jNumber));

    // Boutons 1-9
    for (int i = 1; i < 10; i++) 
      addOperatorButton(String.valueOf(i), (i - 1) % 3, 4 - (i - 1) / 3, 
			Color.BLUE, new OpNumber(state, i));
    // Bouton 0
    addOperatorButton("0", 0, 5, Color.BLUE, new OpNumber(state, 0));

    // Changement de signe de la valeur courante
    addOperatorButton("+/-", 1, 5, Color.BLUE, new OpChange(state));

    // Operateur point (chiffres apres la virgule ensuite)
    addOperatorButton(".", 2, 5, Color.BLUE, new OpComma(state));

    // Operateurs arithmetiques a deux operandes: /, *, -, +
    addOperatorButton("/", 3, 2, Color.RED, new OpDiv(state));
    addOperatorButton("*", 3, 3, Color.RED, new OpMult(state));
    addOperatorButton("-", 3, 4, Color.RED, new OpMinus(state));
    addOperatorButton("+", 3, 5, Color.RED, new OpPlus(state));

    // Operateurs arithmetiques a un operande: 1/x, x^2, Sqrt
    addOperatorButton("1/x", 4, 2, Color.RED, new OpInv(state));
    addOperatorButton("x^2", 4, 3, Color.RED, new OpSq(state));
    addOperatorButton("Sqrt", 4, 4, Color.RED, new OpRoot(state));

    // Entree: met la valeur courante sur le sommet de la pile
    addOperatorButton("Ent", 4, 5, Color.RED, new OpEnt(state));
